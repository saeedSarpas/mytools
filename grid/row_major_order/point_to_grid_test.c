...
Ensure(point_to_grid, returns_right_results_for_a_trivial_input)
{
  ...
  for(i = 0; i < GRIDX; i += 3)
    for(j = 0; j < GRIDY; j += 3)
      for(k = 0; k < GRIDZ; k++){
        pos[0] = i * (box_lengths[0] / grid_dims[0] * (1 + GRIDX / 100.0));
        pos[1] = j * (box_lengths[1] / grid_dims[1] * (1 + GRIDY / 100.0));
        pos[2] = k * (box_lengths[2] / grid_dims[2] * (1 + GRIDZ / 100.0));
        grid_index = point_to_grid(pos, box_lengths, grid_dims);
